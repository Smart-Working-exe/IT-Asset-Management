...
@section('sub_header')
    ...
    @if($user == 1)
        ...
        <div class="row"> <!-- Wegen Row ist die Progressbar abgeschnitten -->
            @if($room != 'Lager')
                <p class="col-3"><b>IP-Adressbereich: {{$ip['ip-adressbereich_beginn']}}/@if($ip['anzahl_ip'] <= 4)
                            30
                        @elseif($ip['anzahl_ip'] <= 8)
                            29
                        @elseif($ip['anzahl_ip'] <= 16)
                            28
                        @elseif($ip['anzahl_ip'] <= 32)
                            27
                        @elseif($ip['anzahl_ip'] <= 64)
                            26
                        @elseif($ip['anzahl_ip'] <= 128)
                            25
                        @elseif($ip['anzahl_ip'] <= 256)
                            24
                        @elseif($ip['anzahl_ip'] <= 512)
                            23
                        @endif</b></p>
                <div class=" mt-1 col-2">
                    <div class="progress position-relative">
                        <div class="progress-bar"
                             style="width:{{((int)$ip['belegung_ip']/(int)$ip['anzahl_ip'])*100}}%; background-color: {{$colorIP}}"
                             aria-valuenow="40" aria-valuemin="0" aria-valuemax="30">
                            <small class="justify-content-center d-flex position-absolute w-100"
                                   style="color: black">{{$ip['belegung_ip']}}/{{$ip['anzahl_ip']}}</small></div>
                    </div>
                </div>
                ...
                <div class="mt-1 col-2 offset-1">
                    <div class="progress position-relative">
                        <div class="progress-bar"
                             ...
                             aria-valuenow="40" aria-valuemin="0" aria-valuemax="30">
                            <small class="justify-content-center d-flex position-absolute w-100"
                                   style="color: black">{{$cur_belegung}}/{{$max_belegung}}</small></div>
                    </div>
                </div>
                <div class="col-2 offset-lg-9">
                    <form action="/raumansicht?raum={{$room}}" method="post">
                        <input style="height:25px; width:120px;" class="form-control" type="number" id="deviceName"
                               placeholder="Menge" name="belegung" min="0" max="{{$max_belegung}}">
                        <input style="background: transparent; border: none !important; font-size:0; margin-left: -9999999px;{{--Bester Bugfix ever haha--}}"
        ...
        <div class="row"> <!-- Wegen Row ist die Progressbar abgeschnitten -->
            @if($room != 'Lager')
                ...
                <div class=" mt-1 col-2">
                    <div class="progress position-relative">
                        <div class="progress-bar"
                             style="width:{{((int)$ip['belegung_ip']/(int)$ip['anzahl_ip'])*100}}%; background-color: green{{--$color--}}"
                             aria-valuenow="40" aria-valuemin="0" aria-valuemax="30">
                            <small class="justify-content-center d-flex position-absolute w-100"
                                   style="color: black">{{$ip['belegung_ip']}}/{{$ip['anzahl_ip']}}</small></div>
                    </div>
                    ...
                    <div class="modal fade" id="editDevice{{$geraet['id']}}" tabindex="-1" aria-labelledby="editDevice"
                        ...
                        <div class="modal-dialog modal-xl modal-dialog-centered">
                                            ...
                                            <div class="col">
                                                <select class="form-select" data-mdb-clear-button="true"
                                                    ...
                                                    @foreach($raueme as $raum)
                                                        <option value="{{$raum['raumnummer']}}"
                                                                @if($raum['raumnummer'] == $geraet['raumnummer']) selected @endif>{{$raum['raumnummer']}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                            ...
                            <div class="modal-footer justify-content-between">
                                <button type="submit" name="submit_delete" value="{{$geraet['id']}}"
